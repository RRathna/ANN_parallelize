...
void genann_train(genann const *ann, double const *inputs, double const *desired_outputs, double learning_rate) {
    ...
    /* First set the output layer deltas. */
    {
        ...
        #pragma omp parallel private(o, t, d)
            {
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        for (j = 0; j < ann->hidden; ++j) { //should not parallelize for now

            double delta = 0;

            for (k = 0; k < (h == ann->hidden_layers-1 ? ann->outputs : ann->hidden); ++k) { //can be parallelized
