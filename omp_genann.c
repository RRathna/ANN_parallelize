 ...
 *
 * Edited on 7/05/2018 by R. Rathna
 */
...
void genann_train(genann const *ann, double const *inputs, double const *desired_outputs, double learning_rate) {
    ...
    /* Train the outputs. */
    {
        ...
        for (j = 0; j < ann->outputs; ++j) {
            ...
            for (k = 0; k < t; ++k) {
                if (k == 0) {
                    w[k] += *d * learning_rate * -1.0;
                } else {
                    w[k] += *d * learning_rate * i[k-1];
                }
            }
            w += t;
            ++d;
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        int t = (h == 0 ? ann->inputs : ann->hidden) + 1;
        
        for (j = 0; j < ann->hidden; ++j) {
            #pragma omp parallel for
            for (k = 0; k < t; ++k) {
                if (k == 0) {
                    w[k] += d[0] * learning_rate * -1.0;
                } else {
                    w[k] += d[0] * learning_rate * i[k-1];
                }
            }
            w += t;
            ++d;
...
double const *genann_run(genann const *ann, double const *inputs) {
    double const *w = ann->weight;
    double const *ww = ann->weight;
    double *o = ann->output + ann->inputs;
    double *oo = ann->output + ann->inputs;
    double const *i = ann->output;
    double const *ii = ann->output;

    /* Copy the inputs to the scratch area, where we also store each neuron's
     * output, for consistency. This way the first layer isn't a special case. */
    memcpy(ann->output, inputs, sizeof(double) * ann->inputs);

    int h, j, k,count_w = 0, count_o = 0; //count to track the offsset of w after the loops
    int offset =0;
    const genann_actfun act = ann->activation_hidden;
    const genann_actfun acto = ann->activation_output;
    //printf("ann->hidden_layers : %f; ann->hidden : %f \n",ann->hidden_layers, ann->hidden);
    ...
    for (h = 0; h < ann->hidden_layers; ++h) {
        ...
        for (j = 0; j < ann->hidden; ++j) {
            ...
            double sums = *ww++ * -1.0;
            if (h > 0) {
                offset = (ann->hidden)*(ann->inputs+1)  + (h-1)*ann->hidden*(ann->hidden+1) + (j)*(ann->hidden +1);
            }
            else {
                offset = j*(ann->inputs + 1);
            }
            sum = w[offset] * -1.0;
            
            //printf("sum : %f, sums : %f",sum,sums);
            int t = (h == 0 ? ann->inputs : ann->hidden);
            for (k = 0; k < t; ++k) {
                sum += w[(ann->hidden)*(ann->inputs+1)  + (h-1)*ann->hidden*(ann->hidden+1) + (j)*(ann->hidden +1) + k+1] * i[k];
                sums += *ww++ * i[k];
                //printf ("i[k] %f, ii[k] %f \n",i[k],ii[k] );
                count_w++;
            } //this loop just pushed w forward by t
            
            o[h*ann->hidden + j] = act(sum);
            *oo++ = act(sums);
            //printf("o : %f, oo : %f\n",o[h*ann->hidden + j],*(oo-1));
            count_o++;
        }
        //printf("count_w : %d, ann->hidden_layers : %d, ann->hidden: %d \n",count_w, ann->hidden_layers, ann->hidden );

        i = ann->output + (h == 0? ann->inputs:(ann->inputs + (h)*ann->hidden));
        //printf(" i : %f, ii : %f\n", i, ii);
    }
    offset = (ann->hidden)*(ann->inputs+1)  + (ann->hidden_layers-1)*ann->hidden*(ann->hidden+1);
    //
    //printf("count_w : %d, offset : %d\n",count_w, offset);
    //printf("count_o : %d, ann->hidden_layers*ann->hidden : %d\n",count_o, ann->hidden_layers*ann->hidden);
    //exit(0);
    w = w + offset;
    o = o + ann->hidden_layers*ann->hidden;
    double const *ret = o;
